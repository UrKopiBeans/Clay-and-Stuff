...
/* Same helpers as quotation.php/my-orders.php so figure details look consistent here too. */
...
/* GET ONLY THE ORDERS WAITING FOR PAYMENT VERIFICATION — dito lang
   lalabas yung kailangang husgahan (approve/reject), di na kailangang
   maghanap sa lahat ng order sa Bookings.

   "Balance Payment" filter — staff-verified na balance payments,
   naghihintay ng owner's final sign-off bago maging "to_ship"
   ("for_balance_approval" status, gaya ng "for approval" pero para
   sa balance sub-flow. Tingnan ang helpers/order_status_helper.php).

   Parehong pattern ng filter dropdown ng Active Booking ("All /
   Processing / Revision") — "afilter" GET param, naka-default sa
   "All", may bilang bawat option. */

$activeFilter = isset($_GET["afilter"]) ? strtolower(trim($_GET["afilter"])) : "all";

if (!in_array($activeFilter, ["all", "first", "balance"], true)) {
    $activeFilter = "all";
}

$firstApprovalOrders   = [];
$balanceApprovalOrders = [];

$approvalOrdersSelect =
    "SELECT o.order_id, o.order_type, o.order_method, o.booking_date, o.rush_fee,
            ...
            o.balance_to_pay, o.balance_payment_reference, o.balance_proof, o.balance_paid_at,
...
$firstApprovalStatusDb = "for approval";

$stmt = $conn->prepare($approvalOrdersSelect);
$stmt->bind_param("s", $firstApprovalStatusDb);
$stmt->execute();

$firstApprovalResult = $stmt->get_result();

while ($row = $firstApprovalResult->fetch_assoc()) {
    $row["list_source"]      = "first";
    $firstApprovalOrders[] = $row;
}

$stmt->close();

$balanceApprovalStatusDb = "for_balance_approval";

$stmt = $conn->prepare($approvalOrdersSelect);
$stmt->bind_param("s", $balanceApprovalStatusDb);
$stmt->execute();

$balanceApprovalResult = $stmt->get_result();

while ($row = $balanceApprovalResult->fetch_assoc()) {
    $row["list_source"]        = "balance";
    $balanceApprovalOrders[] = $row;
}

$stmt->close();

$firstApprovalCount   = count($firstApprovalOrders);
$balanceApprovalCount = count($balanceApprovalOrders);

if ($activeFilter === "first") {

    $orders = $firstApprovalOrders;

} elseif ($activeFilter === "balance") {

    $orders = $balanceApprovalOrders;

} else {

    // "All" — pagsamahin ang dalawa, pinaka-matagal munang naghihintay
    $orders = array_merge($firstApprovalOrders, $balanceApprovalOrders);

    usort($orders, function ($a, $b) {

        $aTime = strtotime($a["paid_at"] ?? $a["created_at"]);
        $bTime = strtotime($b["paid_at"] ?? $b["created_at"]);
...
/* GET FIGURES + IMAGES FOR EACH ORDER — para makita ni staff kung
   ano talaga ang inaprubahan bago i-verify ang bayad. */
...
foreach ($orders as &$order) {
    ...
    // DEFENSIVE FALLBACK — kung wala pang hirono_box_design/blind_items
    // columns, mabibigo ang prepare() (false, depende sa mysqli report
    // mode). Fallback sa lumang column set imbes na sumabog ang page.
    ...
    while ($frow = $fresult->fetch_assoc()) {
        ...
        // Kung fallback query gamit tayo, wala pa ring hirono_box_design/
        // blind_items dito — punan ng safe default.
        if (!$figuresHaveExtendedHironoColumns) {
        ...
        // BOX REFERENCE IMAGES (Hirono Blind Box only) — kung wala pang
        // order_figure_box_images table, tahimik lang itong mabibigo.
        ...
        // Decode into [label, price] pairs para sa display.
...
/* Wala nang AUTO-SELECT ng unang order sa listahan, parehong
   pattern ng Quotation/Verification pages. */
...
<title>Figurify — For Approval</title>

<style>
/* For-Approval page's own stylesheet. Kept separate from quotation/verification/active-booking
   (even though they look alike) so editing one doesn't break the others.
   Copied from my-orders.css, owner/orders.css, and owner/quotation.css. */

/* ---------- [1] FROM my-order/my-orders.css ---------- */
/* My Orders redesign - same data/flow, new layout based on order_preview_updated_v3.html */
...
/* PAGE SCROLL (desktop/tablet) — normal page scroll (hindi
   no-scroll shell) para maabot ang footer. Order list (kaliwa) at
   order summary (kanan) sticky pa rin habang nag-scroll. */
...
/* Detail item card - reusable "line item" (image + label + value + price), used for both Figure Details and Custom Box. */
...
/* Shared orders.css (Owner + Staff) — dating hiwalay na kopya,
   iisa na lang dito para di mag-drift ang dalawa. */
...
/* Shared quotation.css - 3-column workspace (order list / detail / summary).
   Classes use "qv-" prefix so they don't clash with the generic names in my-orders.css/orders.css. */

/* Search bar — nasa loob ng ".qv-toolbar" (parehong pattern ng
   Quotation page kahit walang tabs dito, para magkatugma ang
   markup/CSS scaffolding ng dalawang pahina). */
.qv-toolbar{
    margin-top: 8px;
    display: flex;
    align-items: center;
...
/* QUOTATION WORKSPACE — client wanted colored background dito
   (hindi plain white ng default .card), kaya may override. */

/* .qv-workspace stays transparent on all qv- pages - only .qv-list-panel and
   .qv-right-col get the card look, so it doesn't look like a box inside a box. */
.qv-workspace{
    background: transparent;
    border: none;
    outline: none;
...
/* Fixed-height layout: just 2 containers, .qv-list-panel (left) and .qv-right-col (right).
   The .app/.main/.qv-workspace/.qv-shell chain fits 100vh via flexbox so there's no page
   scroll, only the two panels scroll internally. On mobile (<=1180px) it stacks and scrolls normally. */
...
/* form wraps center + right columns so it can post everything together.
   display:contents so the <form> itself doesn't break the grid layout. */
.qv-shell form{
    display: contents;
}
...
/* Mobile/tablet: nag-sstack sa 1 column (normal page scroll). */
...
/* Table layout ang order list, .qv-list may internal scroll, sticky ang header row. */
.qv-list{
    flex: 1 1 auto;
    min-height: 0;
    overflow: auto;
...
/* Order Information card - same look as the customer's My Orders page (icon chips in a grid), so it's consistent across Staff, Owner, and Customer. */
...
/* ---------- FIGURE DETAILS + CUSTOM BOX SIDE BY SIDE ----------
   Kapareho ng "viewer-grid" sa Customer My Orders page — Figure
   Details sa kaliwa, Custom Box sa kanan, magkatabi sa desktop. */

/* Figure Details and Custom Box stacked (not side-by-side) - same as Quotation,
   the column's too narrow now to fit both side by side without looking cramped. */
.qv-viewer-grid{
    display: flex;
    flex-direction: column;
    gap: 16px;
...
/* Detail item card - line item row (image/icon + label + value + price), same style as Customer My Orders' Figure Details. */
...
/* Makes an item span the full width of the grid row on its own
   (used for Date and Letter sa Custom Box Details), para hindi na
   sila kakatabe ng ibang field na hindi related. */
.qv-spec-full{
    grid-column: 1 / -1;
}

/* Pinipilit magsimula ng bagong row (ilalagay sa unang Optional
   Extra), para magkakatabi silang lahat sa sarili nilang row at
   hindi na sila madikit/nakikialam sa Date sa row bago nila. */
.qv-row-start{
    grid-column-start: 1;
}
...
/* Per-figure read-only total (Verification page only) - shows "Price For This
   Figure" inside the figure panel, unlike Quotation where it's only in the summary sidebar. */
...
/* Empty state when customer didn't add a custom box - dashed slot instead of
   nothing, same as "No Custom Box Added" in Customer My Orders. */
.qv-box-card-empty{
    border: 1px solid var(--gray-light);
    outline: none;
    box-shadow: none;
...
/* Summary gets its own box (border/shadow) to visually separate it from the
   order detail above. Still inside one scrolling .qv-right-col, no nested scroll. */
.qv-summary{
    display: flex;
    flex-direction: column;
    flex-shrink: 0;
...
/* Thin, subtle scrollbar — ang .qv-list (left, order list) at
   ang .qv-right-col (right, buong detail + summary) na lang ang
   dalawang bahagi ng workspace na may sariling scroll. */
...
/* Shared dashboard styles - owner.css/staff.css just @import this, role-specific stuff stays in those files. */
...
/* Duplicate ".card" rule (dating nag-o-override, gray border) ay
   tinanggal na para tumugma sa ibang dashboard pages. */
...
/* Status colors - same source as Staff Dashboard, Calendar, and customer My Orders (Admin/order_status_helper.php). */
...
/* Dashboard calendar (#calendarDays) ay read-only preview lang —
   pointer-events:none para di mag-trigger ng :hover. */
#calendarDays .day {
    pointer-events: none;
}
...
/* Completed orders table - separate from .booking-row since this has 6 columns (Customer + Courier), not 5. Same as Staff Dashboard. */
...
<!-- Uses the same "qv-shell" 3-column workspace as Quotation and Payment Verification,
     so all three pages share styles from quotation.css and look the same. -->
...
<main class="main">
    ...
    <!-- For-Approval workspace - same 3-column shell as Quotation/Payment Verification,
         but the right panel here is for the final Approve/Reject before Processing. -->
        ...
        <div class="qv-shell">
            ...
            <div class="qv-list-panel">
                ...
                <div class="qv-toolbar">
                    ...
                    <?php
                    /* filter dropdown, parehong itsura/pattern ng Active Booking page's "All / Processing / Revision" filter */
                    $faFilterLabels = [
                        "all"     => "All (" . (int) ($firstApprovalCount + $balanceApprovalCount) . ")",
                        "first"   => "First Payment (" . (int) $firstApprovalCount . ")",
                        "balance" => "Balance Payment (" . (int) $balanceApprovalCount . ")",
                    ];
                    $faFilterCurrentLabel = $faFilterLabels[$activeFilter] ?? $faFilterLabels["all"];
                    ?>

                    <div class="qv-filter-dropdown" id="faFilterDropdown">

                        <button
                            type="button"
                            class="qv-filter-trigger"
                            onclick="figurifyToggleFilterDropdown('faFilterDropdown')"
                        >
                            <?php echo e($faFilterCurrentLabel); ?>
                        ...
                        <div class="qv-filter-menu">
                            ...
                            <a
                                href="for-approval.php?afilter=all"
                                class="qv-filter-option<?php echo ($activeFilter === "all") ? " active" : ""; ?>"
                            >
                                All (<?php echo (int) ($firstApprovalCount + $balanceApprovalCount); ?>)
                            </a>

                            <a
                                href="for-approval.php?afilter=first"
                                class="qv-filter-option<?php echo ($activeFilter === "first") ? " active" : ""; ?>"
                            >
                                First Payment (<?php echo (int) $firstApprovalCount; ?>)
                            </a>

                            <a
                                href="for-approval.php?afilter=balance"
                                class="qv-filter-option<?php echo ($activeFilter === "balance") ? " active" : ""; ?>"
                            >
                                Balance Payment (<?php echo (int) $balanceApprovalCount; ?>)
                            </a>
                        ...
                        <table class="qv-table">
                            ...
                            <tbody>
                                ...
                                <?php foreach ($orders as $order): ?>
                                    ...
                                    $qvRowHref    = "for-approval.php?afilter=" . e($activeFilter) . "&order_id=" . (int) $order["order_id"];
                                    ?>
            ...
            <?php if (!$selectedOrder): ?>
                ...
                <div class="qv-right-col">
                    ...
                    <div class="qv-summary">
                        <div class="qv-summary-head"><h2>For Approval</h2></div>
                        <div class="qv-summary-body">
                            <div class="qv-empty">Nothing to approve yet.</div>
                        </div>
                    </div>
            ...
            <?php else: ?>
                    ...
                    <div class="qv-detail-col">
                        ...
                        <!-- Order Information card removed - those fields already show
                             as columns in the order list on the left. -->


                        <!-- Customer Order / Payment toggle removed - Figure Details is always
                             visible now. Payment/Shipping and Refund details are below,
                             after qv-figure-panels. -->

                    <!-- Customer order view (figures) - same qv-figure-card/qv-box-card design as
                         Quotation, but read-only here since this page is just for reviewing
                         before verifying payment, no editable price input. -->
                        ...
                        <div class="qv-figure-panels">
                            ...
                            <?php foreach ($selectedOrder["figures"] as $qIndex => $fig): ?>
                                ...
                                <?php
                                // BAWAT FIGURE, HIWA-HIWALAY NA "SUBOK" (try/catch +
                                // ob_start) — para kapag may error sa isang figure,
                                // doon lang ito lalabas, di masisira ang buong tab.
                                try {
                                ob_start();
                                ...
                                >
                                    ...
                                    <?php else: ?>
                                        ...
                                        <!-- No custom box added for this figure - still show the
                                             card but empty, same as Quotation's empty state. -->
                                ...
                                echo ob_get_clean();
                                    ...
                                    /* may nangyaring error habang ginagawa ang
                                       figure na ito — huwag na lang ituloy ang
                                       kalahating-nagawang HTML nito */
                                    if (ob_get_level() > 0) {
                                        ob_end_clean();
                                    }
                                    ?>
                    ...
                    <!-- PAYMENT & SHIPPING DETAILS + REFUND/RETURN DETAILS -->
                        ...
                        <?php
                        $isBalanceTab = (strtolower(trim((string) $selectedOrder["status"])) === "for_balance_approval");

                        $verifyRefValue   = $isBalanceTab ? $selectedOrder["balance_payment_reference"] : $selectedOrder["payment_reference"];
                        $verifyProofValue = $isBalanceTab ? $selectedOrder["balance_proof"] : $selectedOrder["payment_proof"];
                        $verifyPaidAt     = $isBalanceTab ? $selectedOrder["balance_paid_at"] : $selectedOrder["paid_at"];
                        ?>

                        <div class="staff-payment-details">

                            <div class="staff-payment-details-title">
                                <?php echo $isBalanceTab ? "Customer's Balance Payment Details" : "Customer's Payment &amp; Shipping Details"; ?>
                            </div>
                            ...
                            <div class="staff-payment-details-grid">
                                ...
                                <?php if ($isBalanceTab): ?>
                                    ...
                                    <span>
                                        ...
                                        ₱<?php echo number_format((float) $selectedOrder["balance_to_pay"], 2); ?>
                                    </span>
                                ...
                                <span>
                                    ...
                                    <?php echo e($verifyRefValue); ?>
                                </span>

                                <?php if (!empty($verifyPaidAt)): ?>
                                    <span>
                                        <strong>Submitted:</strong>
                                        <?php echo date("F j, Y — g:i A", strtotime($verifyPaidAt)); ?>
                                    </span>
                                <?php endif; ?>

                            </div>

                            <?php if (!empty($verifyProofValue)): ?>
                                ...
                                <div class="staff-payment-proof">
                                    ...
                                    <img
                                        src="<?php echo $siteBase . e($verifyProofValue); ?>"
                                        alt="Proof of payment for order #<?php echo (int) $selectedOrder["order_id"]; ?>"
                                        class="staff-payment-proof-thumb"
                                        onclick="openImageLightbox('<?php echo $siteBase . e($verifyProofValue); ?>', 'Proof of Payment — Order #<?php echo (int) $selectedOrder["order_id"]; ?>')"
                                    >
                    ...
                    <div class="qv-summary">
                        ...
                        <div class="qv-summary-body">
                            ...
                            <div class="qv-price-breakdown">
                                ...
                                <?php if ($isBalanceTab): ?>
                                    ...
                                    <div class="qv-price-row qv-price-row-downpayment">
                                        ...
                                        <strong>₱<?php echo number_format((float) $selectedOrder["balance_to_pay"], 2); ?></strong>
                                    </div>
                            ...
                            <div class="qv-validity-note">
                                <span>⚠️</span>
                                <div>
                                    <strong>This order was already verified by staff</strong>
                                    <?php echo $isBalanceTab
                                        ? "and is waiting for your final approval before it's marked ready to ship."
                                        : "and is waiting for your final approval before it moves to Processing."; ?>
                                </div>
                            </div>

                            <?php
                            $approvalSource    = $isBalanceTab ? "for_balance_approval" : "for_approval";
                            $approvalModalId   = $isBalanceTab ? "declineModal_for_balance_approval" : "declineModal_for_approval";
                            $approveConfirmMsg = $isBalanceTab
                                ? "Give final approval and mark this order Ready to Ship?"
                                : "Give final approval and move this order to Processing?";
                            $rejectConfirmMsg  = $isBalanceTab
                                ? "Reject this balance payment? The order will go back to Awaiting Balance so the customer can resubmit."
                                : "Reject this order and cancel it? This cannot be undone.";
                            ?>
                            ...
                            <div class="verify-actions" style="margin: 0; padding-bottom: 0; border-bottom: none;">
                                ...
                                <form
                                    ...
                                    onsubmit="return confirm(<?php echo json_encode($approveConfirmMsg); ?>);"
                                >

                                    <input type="hidden" name="order_id" value="<?php echo (int) $selectedOrder["order_id"]; ?>">
                                    <input type="hidden" name="action" value="approve">
                                    <input type="hidden" name="source" value="<?php echo e($approvalSource); ?>">
                                ...
                                <form
                                    ...
                                    onsubmit="return figurifyConfirmDecline(this, <?php echo json_encode($rejectConfirmMsg); ?>);"
                                >
                                    ...
                                    <button type="button" class="verify-btn verify-btn-reject" onclick="figurifyOpenDeclineModal('<?php echo $approvalModalId; ?>')">
                                        Reject
                                    </button>

                                    <div class="decline-modal-overlay" id="<?php echo $approvalModalId; ?>">
                                        <div class="decline-modal-box">
                                            <h3 class="decline-modal-title"><?php echo $isBalanceTab ? "Reject Balance Payment" : "Reject Order"; ?></h3>
                                            <p class="decline-modal-desc">
                                                <?php echo $isBalanceTab
                                                    ? "Pick a reason. The order will go back to \"Awaiting Balance\" and the customer will be notified."
                                            ...
                                            >
                                                ...
                                                <?php if ($isBalanceTab): ?>
                                                    <option value="insufficient_balance_payment">Insufficient Balance Payment</option>
                                                    <option value="no_balance_payment_received">No Balance Payment Received</option>
                                            ...
                                            <div class="decline-modal-actions">
                                                <button type="button" class="decline-modal-btn decline-modal-btn-cancel" onclick="figurifyCloseDeclineModal('<?php echo $approvalModalId; ?>')">Cancel</button>
                            ...
                            <p class="qv-summary-hint">
                                <?php echo $isBalanceTab
                                    ? "Rejecting will send this order back to Awaiting Balance and notify the customer."
                                    : "Rejecting will cancel the order and notify the customer."; ?>
                            </p>
...
/* FILTER DROPDOWN — toggle ng ".qv-filter-dropdown" (All / First Payment / Balance Payment). Isang dropdown lang ang bukas sa isang pagkakataon, at nagsasara ito pag nag-click sa labas o pag pinindot ang Escape. Parehong pattern ng Active Booking/Quotation/Shipments. */
...
/* Switch which figure's details show when order has 2+ figures - only one visible at a time, same as My Orders */
...
function selectFigure(button) {
    ...
    /* May sariling kopya ng figure tabs ang bawat figure panel
       (paulit-ulit na render sa foreach), kaya i-sync ang "active"
       state sa LAHAT ng kopya, hindi lang yung pinindot. */
    panelsWrap
...
/* Left panel search - filters the orders already loaded on this tab, no server request. Same as Quotation/Payment Verification. */
