...
// Shared logic ng owner/ at staff/send_progress_update.php. $figurifyRole ginagamit sa sent_by_role column.
...
// Block any update once a final photo was sent na — backend enforcement ng pagtatago ng "Send Update"
// button sa UI, kahit sino pa ang direktang mag-POST dito.
...
// Block a new update habang hindi pa nasasagot ang current one, o may pending revision request pa.
// Backend check din, hindi lang UI ($canSendUpdate) — para safe kahit sa double-click/stale tab.
...
if ($errorMsg === "") {
    ...
    if (!move_uploaded_file($_FILES["progress_image"]["tmp_name"], $destPath)) {
        ...
        // Insert the progress update row. Final photo -> auto "approved" na ang response
        // (walang Good/Revision step pa), dito na lalabas ang Ship Out button.
        ...
        // Final photo -> ilipat ang order sa "to_ship" status (dito lalabas sa "To Ship" tab)
        if ($is_final) {
            ...
            // I-notify ang ibang Owner/Staff
        ...
        // Email notification (para hindi kailangan nakatutok sa system ang customer)
