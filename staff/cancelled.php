...
/* Same helpers ginagamit sa quotation.php / active-booking.php, para magtugma
   ang Figure Details display dito. */
...
/* shared with owner/cancelled.php — entry file kailangan mag-set ng $figurifyRole at $figurifyPageDir muna */

/* orders that didn't push through — cancelled, declined, or refunded */

$cancelledOrders = [];

/* join users to get the customer name/email for the Customer column */
$result = $conn->query("
    SELECT
        o.order_id,
        o.order_type,
...
/* same select-left, view-right pattern as Quotation/Verification/Shipments */
...
/* cancelled/declined orders page — same qv- workspace pattern as the other qv- pages.
   .qv-workspace is just a layout wrapper, no box styling; .qv-list-panel and
   .qv-right-col are the ones with the card look, to stay consistent. */
.qv-workspace{
    background: transparent;
    border: none;
    outline: none;
...
<section class="card qv-workspace" style="padding: 10px 0 20px 0;">
    ...
    <div class="qv-shell">
        ...
        <!-- right: order detail (removed old Order#/Customer/Email chips, just Figure Details + Payment/Shipping now) -->
...
/* search cancelled/declined orders — same pattern as the other qv- pages */
...
// real booking dates from the DB, same figurifyBookedDates variable as owner.js so both see the same marks
