...
// same as owner/update_active_order.php, just different role check
//
// action values:
// request_balance - popup asking the balance amount, order -> awaiting_balance,
//                    notifies customer (dating "mark_to_ship" ito na diretso sa
//                    to_ship — pinalitan para dumaan muna sa balance payment
//                    sub-flow bago maging "to_ship", tingnan ang
//                    helpers/order_status_helper.php). "to_ship" mismo ay
//                    galing na sa balance approval chain (verify_order.php,
//                    source=balance), hindi na dito.
// ship_out         - popup with tracking/fee, order -> shipped, notifies customer
// complete_order   - order -> completed, notifies customer
// update_status    - manual dropdown pick, no notification
...
if ($order_id > 0 && $newStatus !== "") {
    ...
    // must be "to_ship" already before it can ship out
    ...
    // must be "shipped" already before it can be marked "completed"
    ...
    // order must still be active AND latest revision approved/in_progress
    // before allowing balance request
    ...
    if ($ownerRow && $newStatus !== "") {
        ...
        if ($action === "ship_out") {
            ...
            // try saving with tracking/fee columns, falls back to
            // status-only update kung wala pa yung migration
            try {

                // shipped_at = NOW() para exact yung actual ship out time
        ...
        // notify other owner/staff to refresh their list
        ...
        // notify customer, only for Ship Out and Complete
        ...
        if ($notifyCustomer) {
            ...
            if ($action === "complete_order") {
                ...
                // review_token lets the review link work without login
                $reviewToken = figurify_ensure_review_token($conn, $order_id);
                $reviewUrl   = figurify_review_url($order_id, $reviewToken);

                // in-system notification only, custom email below has the review button
...
// optional redirect_to field so we stay on Shipments page if that's where the form came from
