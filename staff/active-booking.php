...
/* Shared sa owner/active-booking.php at staff/active-booking.php, walang approval
   part dito (verification flow na yun). Set $figurifyRole/$figurifyPageDir bago i-require. */

require_once __DIR__ . "/../helpers/progress_expire_helper.php";

/* auto-approve progress updates na di na-response ng customer sa loob ng 24 hours */
figurify_autoapprove_stale_progress_updates($conn);

/* session/role check, $ownerName, e(), statusLabel() galing na sa staff-header.php */
...
/* get only orders na currently active (approved, ginagawa) — pwede i-Ship Out or Update dito */

$orders = [];

/* try muna with tracking_number/shipping_fee/balance_to_pay columns, fallback kung wala pa sa DB */
...
/* GET FIGURES + IMAGES FOR EACH ORDER — para makita ng staff ang in-order. */
...
/* MAY NAKA-PENDING BANG "REVISION REQUEST" ANG ORDER? — tinitignan ang pinakahuling
   progress update, kung "revision" ang sagot ng customer, REVISION tab ito, di PROCESSING.  */

foreach ($orders as &$order) {

    $order["has_revision"] = false;

    /* "To Ship" na? — base sa totoong o.status column, di na hula mula sa progress update. */
    $order["is_to_ship"] = (strtolower(trim((string) $order["status"])) === "to_ship");

    /* "Shipped" na? — itinatakda pag Ship Out (Tracking Number/Shipping Fee/Balance to Pay). */
...
/* 3-column layout, same as Verification page, ginagamit ng lahat ng tabs */
...
$visibleOrders = array_values(array_filter(
    ...
    function ($order) use ($activeFilter, $requestedActiveOrderId) {
        ...
        // always show the order requested sa URL kahit anong tab ang active
        if ($requestedActiveOrderId > 0 && (int) $order["order_id"] === $requestedActiveOrderId) {
            return true;
        }

        if ($activeFilter === "all") {
            // "All" = Processing + Revision lang, To Ship/Shipped may sariling page
...
/* PROGRESS UPDATE HISTORY — mga photo update na na-send na (Pending/Approved/Revision + note). */
...
if ($selectedOrderId > 0) {
    ...
    /* fallback kung wala pang "auto_approved" column, migration lang ang mawawala */
    if ($progStmt === false) {
...
// order already to_ship/shipped, so hide Send Update / To Ship buttons na
...
// "pending" added here bukod sa "revision" — dati pwede pa mag-Send Update
// ulit kahit di pa sumasagot ang customer, na-fix na yun
...
// kapag approved/in_progress na ang latest, yung susunod na Send Update ay
// yung final photo na, kaya auto-"approved" na lang di na kailangan ng popup
$autoFinalUpdate = in_array($latestResponse, ["approved", "in_progress"], true);

// quick "To Ship" button, para hindi na kailangang magpadala pa ng final photo
$canMarkToShip = $autoFinalUpdate && $canSendUpdate;

// Ship Out base sa totoong o.status ('to_ship'), hindi sa progress update.
// Walang booking-date lock, pwede i-Ship Out anumang oras.
...
<style>
/* Figurify — staff active booking page styles, all merged into one inline block
   so editing this page won't touch others. Copied from my-orders.css +
   staff/orders.css + quotation.css + progress-update.css. */
...
/* Processing / Revision tabs (kaliwa) at search bar (kanan) — same pattern ng
   Quotation page. Tinanggal ang To Ship / Shipped tabs (nasa Shipments page na yon).  */
.qv-toolbar{
    margin-top: 8px;
    display: flex;
    align-items: center;
...
/* FILTER DROPDOWN — same disenyo ng filter dropdown sa Quotation / Shipments pages. */
...
/* .qv-workspace default white na lang dito (Quotation page may colored bg override) */
...
/* Grid na 2x2 ang tabs (di isang row ng apat) para pantay-pantay ang laki ng button
   at may sapat na espasyo, di na kailangang i-truncate ang text.  */
.qv-filter-tabs{
    display: grid;
...
/* PER-FIGURE READ-ONLY TOTAL — kaparehong "Price For This Figure" row ng .qv-total-row
   sa qv-summary, pero nested sa loob ng figure panel (para sa mga read-only pages).  */
...
/* May sarili nang container/box ang Order Actions summary ngayon, kaya tinanggal
...
/* ---------- [4] FROM staff/progress-update.css ---------- */
...
/* PART 2: dashboard.css styles (via staff.css) */
...
/* note: removed the old "#orderView ..." ID selector fallback, laging nananalo
   yun sa class selectors kahit maayos naka-load ang mga ito */

/* view toggle (kaliwa) + Verify/Cancel buttons (kanan), same row */
...
/* override .verify-actions form{flex:1}/.verify-btn{width:100%} from orders.css,
   kasi doon nagmumula ang paglaki ng buttons */
...
<main class="main">
    ...
    <?php
    /* Processing/Revision tabs, nasa tabi na ng search bar sa loob ng .qv-toolbar.
       Reusable function para pareho ang output kahit saan tawagin.
       To Ship/Shipped tabs tinanggal na sa UI (may sariling Shipment page na). */
    ...
    <section class="card <?php echo $useVerificationLayout ? "qv-workspace" : "orders-manager"; ?>" style="padding: 10px 0 20px 0;">
        ...
        <div class="<?php echo $useVerificationLayout ? "qv-shell" : "orders-manager-grid"; ?>">
            ...
            <?php if ($useVerificationLayout): ?>
                ...
                <!-- Processing tab lang — same itsura ng Verification's left panel. Ibang tabs ay nasa staff-order-list pa rin sa ibaba. -->
            ...
            <div class="<?php echo $useVerificationLayout ? "qv-detail-col" : "staff-order-detail"; ?>">
                ...
                <?php else: ?>
                    ...
                    <!-- Customer Order / Payment toggle inalis na — palagi nang nakikita ang buong Figure Details, walang toggle. -->


                    <!-- PROGRESS UPDATE HISTORY — sa PROCESSING tab, lumilipat ito sa RIGHT column (qv-summary).
                         Dito pa rin sa Revision/To Ship/Shipped tabs. -->
                    ...
                    <!-- ================= CUSTOMER ORDER VIEW (FIGURES) =================
                         ...
                         at ang presyo (staff quote kung meron na, o ang estimate ng
                         customer). Kailangan ito para makita ng staff ang BUONG order
                ...
                <div class="qv-summary">
                    ...
                    <div class="qv-summary-body">
                        ...
                        <?php else: ?>
                            ...
                            <!-- Tinanggal na ang "May Kahilingan na Revision" badge — redundant na sa Accept/Decline buttons sa baba. -->
                            ...
                            >
                                ...
                                <?php elseif ($awaitingCustomerResponse): ?>
                                    ...
                                    <!-- Dati pwede pa mag-Send Update ulit kahit di pa sumasagot ang customer.
                                         Ngayon nakatago ang button hangga't "pending", ito namang paalala na lang. -->
...
/*   KEEP SCROLL POSITION ACROSS ORDER CLICKS — parehong
...
/* avoid double-submit — these actions email the customer too, so lock the button on click */

document.querySelectorAll("form").forEach(function (actionForm) {

    actionForm.addEventListener("submit", function (event) {

        /* cancelled na ng ibang handler (e.g. confirm() sa Complete/To Ship), skip lock */
