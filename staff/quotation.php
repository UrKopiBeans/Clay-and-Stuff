<?php
/* Quotation page ng Staff. Moved out of Shared/quotation.php para dito na lang ang staff code */
...
/* Same helpers as my-orders.php para magtugma ang details display */
...
/* kunin muna ang requested order_id kahit di na "pending"/"quoted" ang status,
   read-only na lang siya dahil naka-gate na ang quoting form sa $canQuote */
...
/* kapag may order_id (redirect from send_quotation.php), i-override ang $quoteFilter
   base sa totoong status — fix sa bug na naiiwan sa Pending tab kahit Quoted na */
...
$visibleOrders = array_values(array_filter(
    ...
    function ($order) use ($quoteFilter, $requestedOrderId) {
        ...
        /* only show orders na "pending" or "quoted" pa talaga */
...
/* which order is open on the right side — no auto-select, click a row muna */
...
<style>
/* Figurify — staff quotation page styles, all merged into one inline block
   so editing this page won't touch others. Copied from my-orders.css +
   staff/orders.css + staff/quotation.css. */
...
/* shared orders.css (Owner + Staff) — dating magkahiwalay, iisa na lang para di mag-drift */
...
/* Pending / Quoted tabs at search bar, same row, sa loob ng .qv-toolbar */
.qv-toolbar{
    margin-top: 8px;
    display: flex;
    align-items: center;
...
/* FILTER DROPDOWN — same disenyo ng filter dropdown sa Active Booking / Shipments pages. */
...
/* Naka-stack na ang Figure Details at Custom Box (di na side-by-side) —
   masikip na kasi ang column ngayon sa loob ng 2-container layout.  */
...
/* PER-FIGURE READ-ONLY TOTAL — kaparehong "Price For This Figure" row ng .qv-total-row
   sa qv-summary, pero nested sa loob ng figure panel (para sa mga pages na read-only, hal. Verification).  */
...
/* May sarili nang container/box ang Quotation summary ngayon, kaya tinanggal
    ...
    <section class="card qv-workspace" style="padding: 10px 0 20px 0;">
        ...
        <div class="qv-shell">
                        ...
                        <table class="qv-table">
                            ...
                            <tbody>
                                ...
                                <?php foreach ($visibleOrders as $order): ?>
                                    ...
                                    /* Row click ay palaging select lang, di nagta-toggle off —
                                       hiwalay na "Clear selection" button ang pang-unselect.  */
            ...
            <?php else: ?>
                ...
                /* Puwede lang mag-quote habang "pending" pa. Once naisumite na
                   (status "quoted"), naka-lock na — di na dapat ma-edit ulit.  */
                            ...
                            >
                                ...
                                <div class="qv-figure-card">
                                    ...
                                    <div class="qv-figure-body">
                                        ...
                                        <!-- FIGURE SPECS — same disenyo ng detail-item-card sa Customer My Orders page. -->
                                ...
                                <?php else: ?>
                                    ...
                                    <!-- Walang box add-on — ipinapakita pa rin ang card pero blangko, same empty state ng Customer My Orders. -->
...
/* FILTER DROPDOWN — nagsasara pag nag-click sa labas o Escape. */
...
/* LIVE COMPUTED TOTAL — display lang ito habang nagta-type. Server ang muling
   magko-compute ng totoong total base sa quoted_price na ipinadala (di trusted ang client).  */
...
/* SWITCH WHICH FIGURE'S DETAILS ARE SHOWN (kapag may 2+ figure) */
...
/* note: qvSwapGallery removed here since the big gallery image is gone too.
   Still in owner/quotation.php though, owner side not yet adjusted. */
...
/* LEFT PANEL — quick client-side search, walang extra request sa server. */
...
/* KEEP SCROLL POSITION ACROSS ORDER CLICKS — normal navigation reloads the page,
   kaya i-save/restore ang scroll ng left at right panels.  */
